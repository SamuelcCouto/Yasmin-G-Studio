  ...
  return (
    <Section id="pacotes" tone="noite" space="default" aria-labelledby="pacotes-titulo">
      <Container>
        ...
        <p className="text-luz-suave mt-5 max-w-[52ch]" data-reveal="up">
        ...
        <ul className="mt-12 grid gap-x-12 gap-y-8 sm:grid-cols-2 lg:grid-cols-3">
          {packages.map((item, index) => (
            ...
            >
              <h3 className="font-display text-luz text-[1.35rem]">{item.name}</h3>
              <p className="text-ouro-claro mt-3 text-[1.05rem] tabular-nums">
                {formatPrice(item.totalCents)}
              </p>
              <p className="text-luz-suave mt-1 text-[0.88rem] tabular-nums">
